	...
	def search(self, limit):
		while self.open_nodes != []:
			...
			if self.problem.goal_test(node.state):
				...
				self.average_depth = sum(self.total_depth) / len(self.total_depth)
			...
			for a in self.problem.domain.actions(node.state):
				...
				if not node.in_parent(newstate) and node.depth < limit:
					...
					if node.cost > self.cumulative_cost[0].cost:
						self.cumulative_cost = [node]
					elif node.cost == self.cumulative_cost[0].cost:
						self.cumulative_cost.append(node)
					
					self.total_depth.append(node.depth)
